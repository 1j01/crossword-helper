...
from .puz_export import cells_to_puz, PuzExportError

def output_puzzle(cells: list, format: str, output_path: str | None = None, metadata: dict | None = None):
    metadata = metadata or {}

    if format == 'ascii':
        content = render_grid_ascii(cells)
        if output_path:
        ...
        content = render_grid_html(cells)
        if output_path:
            with open(output_path, 'w', encoding='utf-8') as handle:
                handle.write(content)
        else:
        ...
        try:
            puzzle = cells_to_puz(cells, metadata)
        except PuzExportError as exc:
            logging.error("Failed to export .puz puzzle: %s", exc)
            raise SystemExit(1) from exc
        except Exception as exc:
            logging.exception("Unexpected error while exporting .puz puzzle")
            raise SystemExit(1) from exc
        output_bytes = puzzle.tobytes()
        if output_path:
...
def main():
    ...
    gen_puzzle_parser.add_argument('--format', type=str, choices=['ascii', 'html', 'svg', 'puz'], default='ascii', help='Output format (default: ascii)')
    ...
    gen_puzzle_parser.add_argument('--output', type=str, default=None, help='Write output to this file (default: stdout)')
    gen_puzzle_parser.add_argument('--title', type=str, default='Generated Crossword', help='Puzzle title for puz output (default: Generated Crossword)')
    gen_puzzle_parser.add_argument('--author', type=str, default='Crossword Helper', help='Puzzle author for puz output (default: Crossword Helper)')
    gen_puzzle_parser.add_argument('--copyright', type=str, default='', help='Puzzle copyright for puz output (default: empty)')
    gen_puzzle_parser.add_argument('--notes', type=str, default='Generated by crossword-helper', help='Puzzle notes for puz output (default: Generated by crossword-helper)')
    ...
    gen_drop_off_parser.add_argument('--format', type=str, choices=['ascii', 'html', 'svg', 'puz'], default='ascii', help='Output format (default: ascii)')
    gen_drop_off_parser.add_argument('--output', type=str, default=None, help='Write output to this file (default: stdout)')
    gen_drop_off_parser.add_argument('--title', type=str, default='Drop-Off Puzzle', help='Puzzle title for puz output (default: Drop-Off Puzzle)')
    gen_drop_off_parser.add_argument('--author', type=str, default='Crossword Helper', help='Puzzle author for puz output (default: Crossword Helper)')
    gen_drop_off_parser.add_argument('--copyright', type=str, default='', help='Puzzle copyright for puz output (default: empty)')
    gen_drop_off_parser.add_argument('--notes', type=str, default='Generated by crossword-helper', help='Puzzle notes for puz output (default: Generated by crossword-helper)')
    ...
    if args.command == 'superpuzzition':
        ...
        output_puzzle(cells, args.format, args.output, metadata)
        ...
        if rows is None:
            ...
            if metadata['notes'] == 'Generated by crossword-helper':
                metadata['notes'] = f"Generated by crossword-helper. Key words: {' '.join(args.key_words)}"
            output_puzzle(cells, args.format, args.output, metadata)
    else:
